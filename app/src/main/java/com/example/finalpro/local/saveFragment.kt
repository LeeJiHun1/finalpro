package com.example.finalpro.local
...
import com.example.finalpro.result.SaveAdpater
...
class SaveFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.getLong("SAVE_ID")?.let { saveId ->  //saveId를 전달 받음
            ...
            val pageLiveData: LiveData<PagedList<Location>> = LivePagedListBuilder(
                database.freshDao().loadlocation(saveId = saveId),20).build()
